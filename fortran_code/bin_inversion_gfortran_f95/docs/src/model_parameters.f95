...
contains
   ...
   !!
   !!  Read and stores in memory properties of the fault plane.
   !!
   implicit none
...
!
   nx_c = max(int(nx_p/2+0.51), 1)
   ny_c = max(int(ny_p/2+0.51), 1)
...
!
   ...
   !!
   !!  Save output model to a text file.
   !!
...
133  format(f14.6, f14.6, f14.6, f14.6, f14.6, f14.6, f14.6, f14.6, f14.6, f14.6, e14.6)
   ...
   !!
   !!  Define space of feasible kinematic models.
   !!
   implicit none
   integer :: nx0, ny0
   ...
   open(17,file='bound.in')
   read(17,*) io_surf
   if (io_surf .eq. 1) then
      open(16,file='surface.constrain')
