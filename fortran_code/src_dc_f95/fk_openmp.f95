...
subroutine sub_bs_dc(nx, x, t0, green, disp, nt, ndis)
    use omp_lib
    implicit none
    real(kind=8), intent(in) :: x(ndis), t0(ndis) ! Change kind=8 to kind=4 for x and t0
    complex(kind=8), intent(inout) :: green(nt, 8, ndis)
    logical, intent(in) :: disp
    integer, intent(in) :: nx, nt, ndis
    integer :: i, j, l, nfft, nfft2, nfft3, n, ix, tenPerc, count, total
    integer :: nCom, wc, wc1, wc2, smth, idx0, flip, new_old(9), stype, updn
    real(kind=8) :: k, omega, dk, dw, sigma, const, phi, hs, xmax, vs
    real(kind=8) :: dfac, pmin, pmax, kc, taper, filter, z2
    integer :: ixx
    complex(kind=16) :: att, w, w2, nf1, nf2, u(3,3), aux(9, ndis)
    real(kind=8) :: aj0, aj1, aj2, z, tdata(2*nt)
    complex(kind=16), allocatable :: summ(:, :, :)
    complex(kind=16) :: data1(nt), kahan_y(9, ndis), kahan_t(9, ndis), kahan_c(9, ndis)
    complex(kind=8) :: u_intermediary(3,3)
    logical :: dynamic
    INTEGER :: ii, jj
    

    ! Explicitly declare all variables to match expected types
    stype = 2
    updn = 0

    allocate(summ(9,ndis,nt))
    ! Initialize new_old array and other necessary initializations
...
!*************** do wavenumber integration for each frequency
   z = pmax*nfft2*dw/kc
   k = sqrt(z*z+1)
   total = INT(nfft2*(kc/dk)*0.5*(k+LOG(k+z)/z))
!   write(0,'(a3,f9.5,a8,f9.2,a8,i9)')'dk',dk,'kmax',kc,'N',total
   tenPerc = INT(0.1*total)
   count = 0.
   kc = kc*kc
   data1(:) = cmplx(0.,0.)
