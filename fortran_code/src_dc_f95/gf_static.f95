...
program gf_static
   ...
   real(kind=8) :: ang_d(ndis), dip, theta, azi(ndis)
   real lat_s(9000),lon_s(9000), area
   real lat_sta,lon_sta,lat_p,lon_p
   real(kind=4) dist(ndis)
   real(kind=4) t0(ndis)  ! Assuming subroutine sub_bs_dc expects REAL(kind=4) for t0
   real(kind=8) az,baz,dis
   real ta0,dta
   real(kind=8) :: coef_v(2,3),coef_r(2,5)
   real(kind=8) dxs,dys,niu
   integer j,ir_max,ir,no,ix,iy
   integer iys,ixs,iyp,ixp,nxp,nyp,k,msou
   real(kind=4) green_p(nt,8,ndis)
   real(kind=8) green_sub_dip(3), gf_dip
   real(kind=8) green_sub_stk(3), gf_stk
...
!            kahan_t(:,:) = 0.0
            do iyp = 1, nyp
               ...
               call update_model(dep_p, 0.0d0)  ! Ensure both arguments are of type REAL(8)
